...
export class Plug {
  ...
  doPlugUpdate!: rxjs.Subject<void>;
  ...
  constructor(
    ...
    this.doPlugUpdate = new rxjs.Subject();
    ...
    rxjs.interval(this.deviceRefreshRate * 1000)
      .pipe(operators.skipWhile(() => this.plugUpdateInProgress))
      .subscribe(() => {
        this.refreshStatus();
      });
    ...
    this.doPlugUpdate
      ...
      .subscribe(async () => {
        try {
          ...
          this.errorLog(
            `Plug: ${this.accessory.displayName} failed pushChanges`,
          );
  ...
  private async refreshStatus() {
    try {
      ...
      this.debugLog(
        `Plug: ${this.accessory.displayName} refreshStatus: ${JSON.stringify(
          this.deviceStatus,
        )}`,
      );
      ...
      this.errorLog(
        `Plug: ${this.accessory.displayName} failed refreshStatus with OpenAPI Connection`,
      );
      if (this.deviceLogging.includes("debug")) {
        this.errorLog(
          `Plug: ${this.accessory.displayName} failed refreshStatus with OpenAPI Connection,` +
  ...
  async pushChanges() {
    if (this.On !== this.OnCached) {
      ...
      this.debugLog(
        `Plug: ${this.accessory.displayName} pushchanges: ${JSON.stringify(
          push.data,
        )}`,
      );
  ...
  updateHomeKitCharacteristics() {
    if (this.On === undefined) {
      ...
      this.debugLog(
        `Plug: ${this.accessory.displayName} updateCharacteristic On: ${this.On}`,
      );
    }
  }
  ...
  private statusCode(push: axios.AxiosResponse<{ statusCode: number }>) {
    switch (push.data.statusCode) {
      case 151:
        this.errorLog(
          `Plug: ${this.accessory.displayName} Command not supported by this device type.`,
        );
        break;
      case 152:
      ...
      case 160:
        this.errorLog(
          `Plug: ${this.accessory.displayName} Command is not supported.`,
        );
        break;
      case 161:
      ...
      case 171:
        this.errorLog(
          `Plug: ${this.accessory.displayName} Hub Device is offline. Hub: ${this.device.hubDeviceId}`,
        );
        this.offlineOff();
        break;
      case 190:
      ...
      case 100:
        this.debugLog(
          `Plug: ${this.accessory.displayName} Command successfully sent.`,
        );
        break;
      default:
        this.debugLog(
          `Plug: ${this.accessory.displayName} Unknown statusCode.`,
        );
    }
  }
  ...
  config(device: settings.device & settings.devicesConfig) {
    ...
    if (Object.entries(config).length !== 0) {
      this.warnLog(
        `Plug: ${this.accessory.displayName} Config: ${JSON.stringify(config)}`,
      );
    }
  }
  ...
  refreshRate(device: settings.device & settings.devicesConfig) {
    if (device.refreshRate) {
      this.deviceRefreshRate = this.accessory.context.refreshRate =
        device.refreshRate;
      this.debugLog(
        `Plug: ${this.accessory.displayName} Using Device Config refreshRate: ${this.deviceRefreshRate}`,
      );
    } else if (this.platform.config.options!.refreshRate) {
      this.deviceRefreshRate = this.accessory.context.refreshRate =
        this.platform.config.options!.refreshRate;
      this.debugLog(
        `Plug: ${this.accessory.displayName} Using Platform Config refreshRate: ${this.deviceRefreshRate}`,
      );
    }
  }
  ...
  scan(device: settings.device & settings.devicesConfig) {
    if (device.scanDuration) {
      ...
      if (device.ble) {
        this.debugLog(
          `Plug: ${this.accessory.displayName} Using Device Config scanDuration: ${this.scanDuration}`,
        );
      }
    } else {
      ...
      if (this.device.ble) {
        this.debugLog(
          `Plug: ${this.accessory.displayName} Using Default scanDuration: ${this.scanDuration}`,
        );
      }
    }
  }
  ...
  logs(device: settings.device & settings.devicesConfig) {
    if (this.platform.debugMode) {
      ...
      this.debugLog(
        `Plug: ${this.accessory.displayName} Using Debug Mode Logging: ${this.deviceLogging}`,
      );
    } else if (device.logging) {
      this.deviceLogging = this.accessory.context.logging = device.logging;
      this.debugLog(
        `Plug: ${this.accessory.displayName} Using Device Config Logging: ${this.deviceLogging}`,
      );
    } else if (this.platform.config.options?.logging) {
      this.deviceLogging = this.accessory.context.logging =
        this.platform.config.options?.logging;
      this.debugLog(
        `Plug: ${this.accessory.displayName} Using Platform Config Logging: ${this.deviceLogging}`,
      );
    } else {
      this.deviceLogging = this.accessory.context.logging = "standard";
      this.debugLog(
        `Plug: ${this.accessory.displayName} Logging Not Set, Using: ${this.deviceLogging}`,
      );
    }
  }
