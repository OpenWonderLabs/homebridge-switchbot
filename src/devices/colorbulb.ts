...
export class ColorBulb {
  ...
  Hue!: homebridge.CharacteristicValue;
  HueCached!: homebridge.CharacteristicValue;
  Saturation!: homebridge.CharacteristicValue;
  SaturationCached!: homebridge.CharacteristicValue;
  Brightness!: homebridge.CharacteristicValue;
  BrightnessCached!: homebridge.CharacteristicValue;
  ColorTemperature!: homebridge.CharacteristicValue;
  ColorTemperatureCached!: homebridge.CharacteristicValue;

  // OpenAPI Others
  power: settings.deviceStatus["power"];
  color: settings.deviceStatus["color"];
  brightness: settings.deviceStatus["brightness"];
  colorTemperature: settings.deviceStatus["colorTemperature"];
  ...
  AdaptiveLightingController?:
    | homebridge.ControllerConstructor
    | homebridge.Controller<homebridge.ControllerServiceMap>;
  ...
  doColorBulbUpdate!: rxjs.Subject<void>;
  cacheKelvin!: number;
  ...
  constructor(
    ...
    this.doColorBulbUpdate = new rxjs.Subject();
    ...
    this.debugLog(
      `Color Bulb: ${this.accessory.displayName} adaptiveLightingShift: ${this.adaptiveLightingShift}`,
    );
    if (this.adaptiveLightingShift !== -1) {
      this.AdaptiveLightingController =
        new platform.api.hap.AdaptiveLightingController(this.service, {
    ...
    rxjs
      ...
      .pipe(operators.skipWhile(() => this.colorBulbUpdateInProgress))
      .subscribe(() => {
        this.refreshStatus();
      });
    ...
    this.doColorBulbUpdate
      ...
      .subscribe(async () => {
        try {
          ...
          this.errorLog(
            `Color Bulb: ${this.accessory.displayName} failed pushChanges`,
          );
  ...
  parseStatus() {
    ...
    this.debugLog(
      `Color Bulb: ${this.accessory.displayName} Brightness: ${this.Brightness}`,
    );

    // Color, Hue & Brightness
    if (this.color) {
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} color: ${JSON.stringify(
          this.color,
        )}`,
      );
      const [red, green, blue] = this.color!.split(":");
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} red: ${JSON.stringify(red)}`,
      );
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} green: ${JSON.stringify(
          green,
        )}`,
      );
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} blue: ${JSON.stringify(
          blue,
        )}`,
      );

      const [hue, saturation] = settings.rgb2hs(
        Number(red),
        Number(green),
        Number(blue),
      );
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} hs: ${JSON.stringify(
          settings.rgb2hs(Number(red), Number(green), Number(blue)),
        )}`,
      );
      ...
      this.Hue = hue;
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} Hue: ${this.Hue}`,
      );

      // Saturation
      this.Saturation = saturation;
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} Saturation: ${this.Saturation}`,
      );
    }
    ...
    if (!Number.isNaN(this.colorTemperature)) {
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} OpenAPI ColorTemperature: ${this.colorTemperature}`,
      );
      const mired = Math.round(1000000 / this.colorTemperature!);

      this.ColorTemperature = Number(mired);

      this.ColorTemperature = Math.max(
        Math.min(this.ColorTemperature, 500),
        140,
      );
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} ColorTemperature: ${this.ColorTemperature}`,
      );
    }
  }
  ...
  private async pushHueSaturationChanges() {
    try {
      if (
        this.Hue !== this.HueCached ||
        this.Saturation !== this.SaturationCached
      ) {
        this.debugLog(
          `Color Bulb: ${this.accessory.displayName} Hue: ${JSON.stringify(
            this.Hue,
          )}`,
        );
        this.debugLog(
          `Color Bulb: ${
            this.accessory.displayName
          } Saturation: ${JSON.stringify(this.Saturation)}`,
        );

        const [red, green, blue] = settings.hs2rgb(
          Number(this.Hue),
          Number(this.Saturation),
        );
        this.debugLog(
          `Color Bulb: ${this.accessory.displayName} rgb: ${JSON.stringify([
            red,
            green,
            blue,
          ])}`,
  ...
  updateHomeKitCharacteristics() {
    if (this.On === undefined) {
      ...
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} updateCharacteristic On: ${this.On}`,
      );
    }
    if (this.Brightness === undefined) {
      ...
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} updateCharacteristic Brightness: ${this.Brightness}`,
      );
    }
    if (this.ColorTemperature === undefined) {
      ...
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} updateCharacteristic ColorTemperature: ${this.ColorTemperature}`,
      );
    }
    if (this.Hue === undefined) {
      ...
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} updateCharacteristic Hue: ${this.Hue}`,
      );
    }
    if (this.Saturation === undefined) {
      ...
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} updateCharacteristic Saturation: ${this.Saturation}`,
      );
    }
  }
  ...
  private statusCode(push: axios.AxiosResponse<{ statusCode: number }>) {
    switch (push.data.statusCode) {
      case 151:
        this.errorLog(
          `Color Bulb: ${this.accessory.displayName} Command not supported by this device type.`,
        );
        break;
      case 152:
        this.errorLog(
          `Color Bulb: ${this.accessory.displayName} Device not found.`,
        );
        break;
      case 160:
        this.errorLog(
          `Color Bulb: ${this.accessory.displayName} Command is not supported.`,
        );
        break;
      case 161:
        this.errorLog(
          `Color Bulb: ${this.accessory.displayName} Device is offline.`,
        );
        this.offlineOff();
        break;
      case 171:
        this.errorLog(
          `Color Bulb: ${this.accessory.displayName} is offline. Hub: ${this.device.hubDeviceId}`,
        );
        this.offlineOff();
        break;
      case 190:
      ...
      case 100:
        if (this.platform.debugMode) {
          this.debugLog(
            `Color Bulb: ${this.accessory.displayName} Command successfully sent.`,
          );
        }
        break;
      default:
        if (this.platform.debugMode) {
          this.debugLog(
            `Color Bulb: ${this.accessory.displayName} Unknown statusCode.`,
          );
        }
    }
  }
   ...
   */
  OnSet(value: homebridge.CharacteristicValue) {
   ...
   */
  BrightnessSet(value: homebridge.CharacteristicValue) {
    this.debugLog(
      `Color Bulb: ${this.accessory.displayName} Brightness: ${value}`,
    );
   ...
   */
  ColorTemperatureSet(value: homebridge.CharacteristicValue) {
    this.debugLog(
      `Color Bulb: ${this.accessory.displayName} ColorTemperature: ${value}`,
    );
   ...
   */
  HueSet(value: homebridge.CharacteristicValue) {
   ...
   */
  SaturationSet(value: homebridge.CharacteristicValue) {
    this.debugLog(
      `Color Bulb: ${this.accessory.displayName} Saturation: ${value}`,
    );
  ...
  config(device: settings.device & settings.devicesConfig) {
    ...
    if (Object.entries(config).length !== 0) {
      this.warnLog(
        `Color Bulb: ${this.accessory.displayName} Config: ${JSON.stringify(
          config,
        )}`,
      );
    }
  ...
  refreshRate(device: settings.device & settings.devicesConfig) {
    if (device.refreshRate) {
      this.deviceRefreshRate = this.accessory.context.refreshRate =
        device.refreshRate;
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} Using Device Config refreshRate: ${this.deviceRefreshRate}`,
      );
    } else if (this.platform.config.options!.refreshRate) {
      this.deviceRefreshRate = this.accessory.context.refreshRate =
        this.platform.config.options!.refreshRate;
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} Using Platform Config refreshRate: ${this.deviceRefreshRate}`,
      );
    }
  }
  ...
  logs(device: settings.device & settings.devicesConfig) {
    if (this.platform.debugMode) {
      ...
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} Using Debug Mode Logging: ${this.deviceLogging}`,
      );
    } else if (device.logging) {
      this.deviceLogging = this.accessory.context.logging = device.logging;
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} Using Device Config Logging: ${this.deviceLogging}`,
      );
    } else if (this.platform.config.options?.logging) {
      this.deviceLogging = this.accessory.context.logging =
        this.platform.config.options?.logging;
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} Using Platform Config Logging: ${this.deviceLogging}`,
      );
    } else {
      this.deviceLogging = this.accessory.context.logging = "standard";
      this.debugLog(
        `Color Bulb: ${this.accessory.displayName} Logging Not Set, Using: ${this.deviceLogging}`,
      );
    }
  }

  adaptiveLighting(device: settings.device & settings.devicesConfig) {
  ...
  minStep(device: settings.device & settings.devicesConfig): number {
