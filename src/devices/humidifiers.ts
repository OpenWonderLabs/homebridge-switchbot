...
export class Humidifier {
  ...
  temperatureservice?: homebridge.Service;

  // Characteristic Values
  CurrentRelativeHumidity!: homebridge.CharacteristicValue;
  CurrentTemperature!: homebridge.CharacteristicValue;
  TargetHumidifierDehumidifierState!: homebridge.CharacteristicValue;
  CurrentHumidifierDehumidifierState!: homebridge.CharacteristicValue;
  RelativeHumidityHumidifierThreshold!: homebridge.CharacteristicValue;
  Active!: homebridge.CharacteristicValue;
  WaterLevel!: homebridge.CharacteristicValue;

  // OpenAPI
  auto: settings.deviceStatus["auto"];
  power: settings.deviceStatus["power"];
  humidity: settings.deviceStatus["humidity"];
  lackWater: settings.deviceStatus["lackWater"];
  temperature: settings.deviceStatus["temperature"];
  nebulizationEfficiency: settings.deviceStatus["nebulizationEfficiency"];
  ...
  onState!: settings.serviceData["onState"];
  autoMode!: settings.serviceData["autoMode"];
  percentage!: settings.serviceData["percentage"];
  ...
  doHumidifierUpdate!: rxjs.Subject<void>;
  ...
  constructor(
    ...
    this.doHumidifierUpdate = new rxjs.Subject();
    ...
    if (device.humidifier?.hide_temperature || device.ble) {
      this.debugLog(
        `Humidifier: ${accessory.displayName} Removing Temperature Sensor Service`,
      );
      ...
      this.debugLog(
        `Humidifier: ${accessory.displayName} Add Temperature Sensor Service`,
      );
      ...
      this.debugLog(
        `Humidifier: ${accessory.displayName} Temperature Sensor Service Not Added`,
      );
    }
    ...
    rxjs
      ...
      .pipe(operators.skipWhile(() => this.humidifierUpdateInProgress))
      .subscribe(() => {
        this.refreshStatus();
      });
    ...
    this.doHumidifierUpdate
      ...
      .subscribe(async () => {
        try {
          ...
          this.errorLog(
            `Humidifier: ${this.accessory.displayName} failed pushChanges`,
          );
  ...
  private BLEparseStatus(): void {
    this.debugLog(`Humidifier: ${this.accessory.displayName} BLE parseStatus`);
    // Current Relative Humidity
    this.CurrentRelativeHumidity = this.percentage!;
    this.debugLog(
      `Humidifier: ${this.accessory.displayName} CurrentRelativeHumidity: ${this.CurrentRelativeHumidity}`,
    );
    // Active
    if (this.onState) {
      this.Active = this.platform.Characteristic.Active.ACTIVE;
    ...
    this.debugLog(
      `Humidifier: ${this.accessory.displayName} Active: ${this.Active}`,
    );
  }
  ...
  private openAPIparseStatus() {
    if (this.platform.config.credentials?.openToken) {
      this.debugLog(
        `Humidifier: ${this.accessory.displayName} OpenAPI parseStatus`,
      );
      // Current Relative Humidity
      ...
      if (!this.device.humidifier?.hide_temperature) {
        ...
        this.debugLog(
          `Humidifier: ${this.accessory.displayName} CurrentTemperature: ${this.CurrentTemperature}`,
        );
      }
      ...
      this.debugLog(
        `Humidifier: ${this.accessory.displayName} TargetHumidifierDehumidifierState: ${this.TargetHumidifierDehumidifierState}`,
      );
      this.debugLog(
      ...
      this.debugLog(
        `Humidifier: ${this.accessory.displayName} CurrentHumidifierDehumidifierState: ${this.CurrentHumidifierDehumidifierState}`,
      );
      // Active
      switch (this.power) {
        case "on":
      ...
      this.debugLog(
        `Humidifier: ${this.accessory.displayName} WaterLevel: ${this.WaterLevel}`,
      );
    }
  }
  ...
  private async BLERefreshStatus() {
    ...
    this.device.bleMac = this.device
      ...
      .toLowerCase();
    this.debugLog(
      `Humidifier: ${this.accessory.displayName} BLE Address: ${this.device.bleMac}`,
    );
    try {
      ...
      if (this.connected) {
        ...
        this.errorLog(
          `Humidifier: ${this.accessory.displayName} wasn't able to establish BLE Connection`,
        );
        if (this.platform.config.credentials?.openToken) {
          this.warnLog(
            `Humidifier: ${this.accessory.displayName} Using OpenAPI Connection`,
          );
      ...
      this.errorLog(
        `Humidifier: ${this.accessory.displayName} failed refreshStatus with BLE Connection`,
      );
      if (this.deviceLogging.includes("debug")) {
        this.errorLog(
          `Humidifier: ${this.accessory.displayName} failed refreshStatus with BLE Connection,` +
  ...
  private async openAPIRefreshStatus() {
    if (this.platform.config.credentials?.openToken) {
      this.debugLog(
        `Humidifier: ${this.accessory.displayName} OpenAPI refreshStatus`,
      );
      try {
        this.deviceStatus = (
        ...
        if (this.deviceStatus.message === "success") {
          ...
          this.nebulizationEfficiency =
            this.deviceStatus.body.nebulizationEfficiency;
          this.parseStatus();
          this.updateHomeKitCharacteristics();
        } else {
          this.errorLog(
            `Humidifier: ${
              this.accessory.displayName
            } message: ${JSON.stringify(this.deviceStatus.message)}`,
          );
        }
      } catch (e: any) {
        this.errorLog(
  ...
  private async BLEpushChanges() {
    ...
    if (switchbot !== false) {
      switchbot
        ...
        .then((device_list) => {
          this.infoLog(
            `${this.accessory.displayName} Target Position: ${this.Active}`,
          );
          return device_list[0].percentage(
            this.RelativeHumidityHumidifierThreshold,
          );
        })
        .then(() => {
        ...
        .catch(async (e: any) => {
          this.errorLog(
            `Humidifier: ${this.accessory.displayName} failed pushChanges with BLE Connection`,
          );
          if (this.deviceLogging.includes("debug")) {
            this.errorLog(
              `Humidifier: ${this.accessory.displayName} failed pushChanges with BLE Connection,` +
  ...
  private async openAPIpushChanges() {
    if (this.platform.config.credentials?.openToken) {
      this.debugLog(
        `Humidifier: ${this.accessory.displayName} OpenAPI pushChanges`,
      );
      if (
        this.TargetHumidifierDehumidifierState ===
          this.platform.Characteristic.TargetHumidifierDehumidifierState
        ...
        try {
          this.debugLog(
            `Pushing Manual: ${this.RelativeHumidityHumidifierThreshold}!`,
          );
          const payload = {
            commandType: "command",
            command: "setMode",
    ...
    try {
      if (
        ...
        this.debugLog(
          `Humidifier: ${
            this.accessory.displayName
          } pushAutoChanges: ${JSON.stringify(push.data)}`,
        );
        this.statusCode(push);
      }
    } catch (e: any) {
    ...
    try {
      if (this.Active === this.platform.Characteristic.Active.INACTIVE) {
        ...
        this.debugLog(
          `Humidifier: ${
            this.accessory.displayName
          } pushActiveChanges: ${JSON.stringify(push.data)}`,
        );
        this.statusCode(push);
      }
    } catch (e: any) {
  ...
  updateHomeKitCharacteristics() {
    ...
    if (!this.device.ble) {
      if (this.WaterLevel === undefined) {
        ...
        this.debugLog(
          `Humidifier: ${this.accessory.displayName} updateCharacteristic WaterLevel: ${this.WaterLevel}`,
        );
      }
    }
    ...
    if (this.Active === undefined) {
      ...
      this.debugLog(
        `Humidifier: ${this.accessory.displayName} updateCharacteristic Active: ${this.Active}`,
      );
    }
  ...
  private statusCode(push: axios.AxiosResponse<{ statusCode: number }>): void {
    switch (push.data.statusCode) {
      case 151:
        this.errorLog(
          `Humidifier: ${this.accessory.displayName} Command not supported by this device type.`,
        );
        break;
      case 152:
        this.errorLog(
          `Humidifier: ${this.accessory.displayName} Device not found.`,
        );
        break;
      case 160:
        this.errorLog(
          `Humidifier: ${this.accessory.displayName} Command is not supported.`,
        );
        break;
      case 161:
        this.errorLog(
          `Humidifier: ${this.accessory.displayName} Device is offline.`,
        );
        this.offlineOff();
        break;
      case 171:
        this.errorLog(
          `Humidifier: ${this.accessory.displayName} Hub Device is offline. Hub: ${this.device.hubDeviceId}`,
        );
        this.offlineOff();
        break;
      case 190:
      ...
      case 100:
        this.debugLog(
          `Humidifier: ${this.accessory.displayName} Command successfully sent.`,
        );
        break;
      default:
        this.debugLog(
          `Humidifier: ${this.accessory.displayName} Unknown statusCode.`,
        );
    }
  }
  ...
  private handleTargetHumidifierDehumidifierStateSet(
    ...
    this.debugLog(
      `Humidifier: ${this.accessory.displayName} TargetHumidifierDehumidifierState: ${value}`,
    );
   ...
   */
  private handleActiveSet(value: homebridge.CharacteristicValue) {
  ...
  private handleRelativeHumidityHumidifierThresholdSet(
    ...
    this.debugLog(
      `Humidifier: ${this.accessory.displayName} RelativeHumidityHumidifierThreshold: ${value}`,
    );
  ...
  config(device: settings.device & settings.devicesConfig) {
    ...
    if (Object.entries(config).length !== 0) {
      this.warnLog(
        `Humidifier: ${this.accessory.displayName} Config: ${JSON.stringify(
          config,
        )}`,
      );
    }
  ...
  refreshRate(device: settings.device & settings.devicesConfig) {
    if (device.refreshRate) {
      this.deviceRefreshRate = this.accessory.context.refreshRate =
        device.refreshRate;
      this.debugLog(
        `Humidifier: ${this.accessory.displayName} Using Device Config refreshRate: ${this.deviceRefreshRate}`,
      );
    } else if (this.platform.config.options!.refreshRate) {
      this.deviceRefreshRate = this.accessory.context.refreshRate =
        this.platform.config.options!.refreshRate;
      this.debugLog(
        `Humidifier: ${this.accessory.displayName} Using Platform Config refreshRate: ${this.deviceRefreshRate}`,
      );
    }
  }
  ...
  scan(device: settings.device & settings.devicesConfig) {
    if (device.scanDuration) {
      ...
      if (device.ble) {
        this.debugLog(
          `Humidifier: ${this.accessory.displayName} Using Device Config scanDuration: ${this.scanDuration}`,
        );
      }
    } else {
      ...
      if (this.device.ble) {
        this.debugLog(
          `Humidifier: ${this.accessory.displayName} Using Default scanDuration: ${this.scanDuration}`,
        );
      }
    }
  }
  ...
  logs(device: settings.device & settings.devicesConfig) {
    if (this.platform.debugMode) {
      ...
      this.debugLog(
        `Humidifier: ${this.accessory.displayName} Using Debug Mode Logging: ${this.deviceLogging}`,
      );
    } else if (device.logging) {
      this.deviceLogging = this.accessory.context.logging = device.logging;
      this.debugLog(
        `Humidifier: ${this.accessory.displayName} Using Device Config Logging: ${this.deviceLogging}`,
      );
    } else if (this.platform.config.options?.logging) {
      this.deviceLogging = this.accessory.context.logging =
        this.platform.config.options?.logging;
      this.debugLog(
        `Humidifier: ${this.accessory.displayName} Using Platform Config Logging: ${this.deviceLogging}`,
      );
    } else {
      this.deviceLogging = this.accessory.context.logging = "standard";
      this.debugLog(
        `Humidifier: ${this.accessory.displayName} Logging Not Set, Using: ${this.deviceLogging}`,
      );
    }
  }
