...
export class Fan {
  ...
  constructor(
    ...
    if (device.irfan?.rotation_speed) {
      ...
      this.debugLog(
        `Fan: ${this.accessory.displayName} Rotation Speed Characteristic was removed.`,
      );
    } else {
      // eslint-disable-next-line max-len
      this.debugLog(
        `Fan: ${this.accessory.displayName} Rotation Speed Characteristic was not removed or not added. To Remove Chracteristic, Clear Cache on this Accessory.`,
      );
    }
    ...
    if (device.irfan?.swing_mode) {
      ...
      this.service.removeCharacteristic(characteristic);
      this.debugLog(
        `Fan: ${this.accessory.displayName} Swing Mode Characteristic was removed.`,
      );
    } else {
      // eslint-disable-next-line max-len
      this.debugLog(
        `Fan: ${this.accessory.displayName} Swing Mode Characteristic was not removed or not added. To Remove Chracteristic, Clear Cache on this Accessory.`,
      );
    }
  }
  ...
  private updateHomeKitCharacteristics() {
    if (this.Active === undefined) {
      this.debugLog(
        `Fan: ${this.accessory.displayName} Active: ${this.Active}`,
      );
    } else {
      ...
      this.debugLog(
        `Fan: ${this.accessory.displayName} updateCharacteristic Active: ${this.Active}`,
      );
    }
    if (this.SwingMode === undefined) {
      this.debugLog(
        `Fan: ${this.accessory.displayName} SwingMode: ${this.SwingMode}`,
      );
    } else {
      this.service?.updateCharacteristic(
        this.platform.Characteristic.SwingMode,
      ...
      this.debugLog(
        `Fan: ${this.accessory.displayName} updateCharacteristic SwingMode: ${this.SwingMode}`,
      );
    }
    if (this.RotationSpeed === undefined) {
      this.debugLog(
        `Fan: ${this.accessory.displayName} RotationSpeed: ${this.RotationSpeed}`,
      );
    } else {
      this.service?.updateCharacteristic(
        this.platform.Characteristic.RotationSpeed,
      ...
      this.debugLog(
        `Fan: ${this.accessory.displayName} updateCharacteristic RotationSpeed: ${this.RotationSpeed}`,
      );
    }
  }
  ...
  public async pushTVChanges(payload: payload) {
    try {
      ...
      this.debugLog(
        `Fan: ${this.accessory.displayName} pushTVChanges: ${push.data}`,
      );
      this.statusCode(push);
      this.updateHomeKitCharacteristics();
    } catch (e: any) {
      this.errorLog(
        `Fan: ${this.accessory.displayName} failed pushChanges with OpenAPI Connection`,
      );
  ...
  private statusCode(push: AxiosResponse<{ statusCode: number }>) {
    switch (push.data.statusCode) {
      case 151:
        this.errorLog(
          `Fan: ${this.accessory.displayName} Command not supported by this device type.`,
        );
        break;
      case 152:
      ...
      case 160:
        this.errorLog(
          `Fan: ${this.accessory.displayName} Command is not supported.`,
        );
        break;
      case 161:
      ...
      case 171:
        this.errorLog(
          `Fan: ${this.accessory.displayName} Hub Device is offline. Hub: ${this.device.hubDeviceId}`,
        );
        break;
      case 190:
        this.errorLog(
      ...
      case 100:
        this.debugLog(
          `Fan: ${this.accessory.displayName} Command successfully sent.`,
        );
        break;
      default:
  ...
  config(device: irdevice & irDevicesConfig) {
    ...
    if (Object.entries(config).length !== 0) {
      this.warnLog(
        `Fan: ${this.accessory.displayName} Config: ${JSON.stringify(config)}`,
      );
    }
  }
  ...
  logs(device: irdevice & irDevicesConfig) {
    if (this.platform.debugMode) {
      ...
      this.debugLog(
        `Fan: ${this.accessory.displayName} Using Debug Mode Logging: ${this.deviceLogging}`,
      );
    } else if (device.logging) {
      this.deviceLogging = this.accessory.context.logging = device.logging;
      this.debugLog(
        `Fan: ${this.accessory.displayName} Using Device Config Logging: ${this.deviceLogging}`,
      );
    } else if (this.platform.config.options?.logging) {
      this.deviceLogging = this.accessory.context.logging =
        this.platform.config.options?.logging;
      this.debugLog(
        `Fan: ${this.accessory.displayName} Using Platform Config Logging: ${this.deviceLogging}`,
      );
    } else {
      this.deviceLogging = this.accessory.context.logging = "standard";
      this.debugLog(
        `Fan: ${this.accessory.displayName} Logging Not Set, Using: ${this.deviceLogging}`,
      );
    }
  }
