...
export class Camera {
  ...
  private OnSet(value: CharacteristicValue) {
    this.platform.debug(`Camera: ${this.accessory.displayName} On: ${value}`);
    this.On = value;
    if (this.On) {
      this.pushOnChanges();
    } else {
  ...
  public async pushChanges(payload: payload) {
    try {
      this.platform.log.info(`Camera: ${this.accessory.displayName} Sending request to SwitchBot API. command: ${payload.command},`
  ...
  private statusCode(push: AxiosResponse<{ statusCode: number; }>) {
    switch (push.data.statusCode) {
      case 151:
        this.platform.log.error(`Camera: ${this.accessory.displayName} Command not supported by this device type.`);
        break;
      case 152:
        this.platform.log.error(`Camera: ${this.accessory.displayName} Device not found.`);
        break;
      case 160:
        this.platform.log.error(`Camera: ${this.accessory.displayName} Command is not supported.`);
        break;
      case 161:
      ...
      case 100:
        ...
        break;
      default:
        this.platform.debug(`Camera: ${this.accessory.displayName} Unknown statusCode.`);
    }
  }
