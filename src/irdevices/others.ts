...
export class Others {
  ...
  constructor(
    ...
    if (this.otherDeviceType !== "Fan") {
      ...
      if (this.otherDeviceType === undefined) {
        this.errorLog(
          `Other: ${this.accessory.displayName} No Device Type Set, deviceType: ${device.other?.deviceType}`,
        );
      }
      ...
      this.debugLog(
        `Other: ${accessory.displayName} Fanv2 Service Not Added, deviceType: ${device.other?.deviceType}`,
      );
    }
  }
  ...
  private updateHomeKitCharacteristics() {
    if (this.Active === undefined) {
      this.debugLog(
        `Other: ${this.accessory.displayName} Active: ${this.Active}`,
      );
    } else {
      ...
      this.debugLog(
        `Other: ${this.accessory.displayName} updateCharacteristic Active: ${this.Active}`,
      );
    }
  }
    ...
    if (this.platform.config.options) {
      if (this.device.other) {
        if (this.device.other.commandOn) {
          ...
          this.errorLog(
            `Other: ${this.accessory.displayName} On Command not set, commandOn: ${this.device.other.commandOn}`,
          );
        }
      } else {
        this.errorLog(
          `Other: ${this.accessory.displayName} On Command not set, other: ${this.device.other}`,
        );
      }
    } else {
    ...
    if (this.platform.config.options) {
      if (this.device.other) {
        if (this.device.other.commandOff) {
          ...
          this.errorLog(
            `Other: ${this.accessory.displayName} Off Command not set, commandOff: ${this.device.other.commandOff}`,
          );
        }
      } else {
        this.errorLog(
          `Other: ${this.accessory.displayName} Off Command not set, other: ${this.device.other}`,
        );
      }
    } else {
      this.errorLog(
        `Other: ${this.accessory.displayName} Off Command not set.`,
      );
    }
  }
  ...
  public async pushChanges(payload: payload) {
    try {
      ...
      this.debugLog(
        `Other: ${this.accessory.displayName} pushChanges: ${push.data}`,
      );
      this.statusCode(push);
      this.updateHomeKitCharacteristics();
    } catch (e: any) {
      this.errorLog(
        `Other: ${this.accessory.displayName} failed pushChanges with OpenAPI Connection`,
      );
  ...
  private statusCode(push: AxiosResponse<{ statusCode: number }>) {
    switch (push.data.statusCode) {
      case 151:
        this.errorLog(
          `Other: ${this.accessory.displayName} Command not supported by this device type.`,
        );
        break;
      case 152:
      ...
      case 160:
        this.errorLog(
          `Other: ${this.accessory.displayName} Command is not supported.`,
        );
        break;
      case 161:
        this.errorLog(
          `Other: ${this.accessory.displayName} Device is offline.`,
        );
        break;
      case 171:
        this.errorLog(
          `Other: ${this.accessory.displayName} Hub Device is offline. Hub: ${this.device.hubDeviceId}`,
        );
        break;
      case 190:
        this.errorLog(
      ...
      case 100:
        this.debugLog(
          `Other: ${this.accessory.displayName} Command successfully sent.`,
        );
        break;
      default:
        this.debugLog(
          `Other: ${this.accessory.displayName} Unknown statusCode.`,
        );
    }
  }
  ...
  deviceType(device: irdevice & irDevicesConfig) {
    if (device.other?.deviceType) {
      this.otherDeviceType = this.accessory.context.deviceType =
        device.other.deviceType;
      if (this.deviceLogging === "debug" || this.deviceLogging === "standard") {
        this.warnLog(
          `Other: ${this.accessory.displayName} Using Device Type: ${this.otherDeviceType}`,
        );
      }
    } else {
      this.errorLog(
        `Other: ${this.accessory.displayName} No Device Type Set, deviceType: ${this.device.other?.deviceType}`,
      );
    }
  }
  ...
  config(device: irdevice & irDevicesConfig) {
    ...
    if (Object.entries(config).length !== 0) {
      this.warnLog(
        `Other: ${this.accessory.displayName} Config: ${JSON.stringify(config)}`,
      );
    }
  }
  ...
  logs(device: irdevice & irDevicesConfig) {
    if (this.platform.debugMode) {
      ...
      this.debugLog(
        `Other: ${this.accessory.displayName} Using Debug Mode Logging: ${this.deviceLogging}`,
      );
    } else if (device.logging) {
      this.deviceLogging = this.accessory.context.logging = device.logging;
      this.debugLog(
        `Other: ${this.accessory.displayName} Using Device Config Logging: ${this.deviceLogging}`,
      );
    } else if (this.platform.config.options?.logging) {
      this.deviceLogging = this.accessory.context.logging =
        this.platform.config.options?.logging;
      this.debugLog(
        `Other: ${this.accessory.displayName} Using Platform Config Logging: ${this.deviceLogging}`,
      );
    } else {
      this.deviceLogging = this.accessory.context.logging = "standard";
      this.debugLog(
        `Other: ${this.accessory.displayName} Logging Not Set, Using: ${this.deviceLogging}`,
      );
    }
  }
