...
export class VacuumCleaner {
  ...
  public async pushChanges(payload: payload) {
    try {
      ...
      this.platform.log.error(`Vacuum Cleaner: ${this.accessory.displayName}: failed to push changes,`
        + ` Error Message: ${JSON.stringify(e.message)}`);
      this.platform.debug(`Vacuum Cleaner: ${this.accessory.displayName} Error: ${JSON.stringify(e)}`);
      this.apiError(e);
    }
  }
  ...
  private statusCode(push: AxiosResponse<{ statusCode: number; }>) {
    switch (push.data.statusCode) {
      ...
      case 190:
        ...
        this.platform.log.error(`Vacuum Cleaner: ${this.accessory.displayName} Device internal error due to device states not synchronized with server. Or command fomrat is invalid.`);
        break;
      case 100:
