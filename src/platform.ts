...
export class SwitchBotPlatform implements DynamicPlatformPlugin {
  ...
  async discoverDevices() {
    console.log("Discovering SwitchBot Devices...");
    try {
      if (this.config.credentials?.token) {
        const t = Date.now();
      ...
      this.debugErrorLog(
        `Failed to Discover Devices, Error Message: ${superStringify(e.message)}, Submit Bugs Here: ` + "https://tinyurl.com/SwitchBotBug",
      );
