...
export class MeterPlus extends deviceBase {
  ...
  async BLEparseStatus(serviceData: meterPlusServiceData): Promise<void> {
  ...
  async openAPIparseStatus(deviceStatus: meterPlusStatus): Promise<void> {
    ...
    if (deviceStatus.version) {
      ...
      this.debugSuccessLog(`deviceVersion: ${this.accessory.context.deviceVersion}`);
    }
  }

  async parseStatusWebhook(context: meterPlusWebhookContext): Promise<void> {
