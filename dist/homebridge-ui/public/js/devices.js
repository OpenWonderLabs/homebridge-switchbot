...
export async function addDeviceToConfig(device, options = {}) {
    const { refresh = true, showStatus = true } = options;
    try {
        const { importDiscoveredDevice } = await import('./modals.js');
        const importValues = await importDiscoveredDevice(device);
...
export async function initRemoveAllButton() {
...
export async function loadConfiguredDevices() {
