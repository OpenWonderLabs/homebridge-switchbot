#!/usr/bin/env bash
# Lightweight automated E2E helper for locks.
# Usage:
...
# 3) Run: `bash scripts/e2e/lock-e2e.sh`
...
echo "Running lock E2E script against $HB_URL (accessory $ACCESSORY_ID)"
...
echo
echo "2) Lock"
curl_common POST "/api/accessories/${ACCESSORY_ID}/characteristics" '{ "characteristics": [{"aid":1,"iid":11,"value":1}] }'
sleep 1

echo
echo "3) Unlock"
curl_common POST "/api/accessories/${ACCESSORY_ID}/characteristics" '{ "characteristics": [{"aid":1,"iid":11,"value":0}] }'
